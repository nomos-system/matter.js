...
export function* scanDocument(docRef: HtmlReference) {
    ...
    function* scanPage(ref: HtmlReference, html: Document): Generator<HtmlReference> {
        // Asciidoctor HTML wraps paragraphs in <div class="paragraph"><p>...</p></div>, lists in <li><p>,
        // definitions in <dd><p>, formulas in stemblock/literalblock/listingblock divs.  h7 is non-standard
        // HTML but generated by Asciidoctor for sect6 (6 levels deep) sections
        const elements = html.querySelectorAll(
            "h1, h2, h3, h4, h5, h6, h7, body > p, .paragraph > p, li > p, dd > p," +
                " .stemblock .content, .literalblock .content pre, .listingblock .content pre, table",
        );
        ...
        for (let i = 1; i < elements.length; i++) {
            ...
            switch (element.tagName) {
                ...
                case "H7":
                    {
                        ...
                        // Unrecognized heading (no section number, not a datatype/field pattern).  Add to prose
                        // so it appears as a subsection heading in the details text (e.g. "Fabric-Scoped List")
                        if (text && currentRef) {
                            if (!currentRef.prose) {
                                currentRef.prose = [];
                            }
                ...
                case "P":
                    {
                        // Skip paragraphs inside admonition blocks — they are handled by the TABLE case
                        if (element.closest(".admonitionblock")) {
                            continue;
                        }
                        ...
                        // Skip this check for list items — their text (e.g. "LandmarkInfo field") can false-match
                        // heading patterns and create fake sections that collide with real ones
                ...
                case "DIV":
                case "PRE":
                    // Stemblock/literalblock content — add as prose
                    if (currentRef) {
                        if (!currentRef.prose) {
                            currentRef.prose = [];
                        }
                ...
                case "TABLE":
                    ...
                    // Extract content from Asciidoctor admonition blocks (notes, warnings, etc.).
                    // Mark content elements with a data-admonition attribute so addDocumentation can
                    // format them directly without a two-step marker/content roundtrip
                    {
                        const admonitionBlock = element.closest(".admonitionblock");
                        if (admonitionBlock) {
                            const content = element.querySelector("td.content");
                            if (content) {
                                if (!currentRef.prose) {
                                    currentRef.prose = [];
                                }

                                const admonitionType = admonitionBlock.classList.contains("warning")
                                    ? "WARNING"
                                    : "NOTE";

                                // Add content paragraphs, or wrap raw text in a synthetic <p>
                                const contentPs = content.querySelectorAll("p");
                                const elements =
                                    contentPs.length > 0
                                        ? Array.from(contentPs)
                                        : [
                                              (() => {
                                ...
                                for (const p of elements) {
                                    (p as HTMLElement).setAttribute("data-admonition", admonitionType);
                                    currentRef.prose.push(p as HTMLElement);
                                }
                            }
                            break;
                        }
