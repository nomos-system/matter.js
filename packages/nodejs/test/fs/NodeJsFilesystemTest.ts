...
describe("NodeJsFilesystem", () => {
    ...
    beforeEach(async () => {
        rootDir = await mkdtemp(join(tmpdir(), "matterjs-fs-test-"));
        fs = new NodeJsFilesystem(rootDir);
    });
    ...
    describe("File operations", () => {
        ...
        it("stat returns file info", async () => {
            ...
            expect(s.type).equal("file");
            expect(s.size).equal(5);
            expect(s.mtime).instanceOf(Date);
        });
    ...
    describe("Directory operations", () => {
        ...
        it("mkdir is recursive", async () => {
            const dir = fs.directory("a");
            const subDir = dir.directory("b").directory("c");
            await subDir.mkdir();
            expect(await subDir.exists()).equal(true);
        });
    ...
    describe("Nested paths", () => {
        it("file in subdirectory with pre-created parent", async () => {
            const dir = fs.directory("a");
            await dir.mkdir();
            const subDir = dir.directory("b");
            await subDir.mkdir();
            const file = subDir.file("deep.txt");
