...
/**
 * Definitions for the ServiceArea cluster.
 *
 * This cluster provides an interface for controlling the areas where a device should operate, for reporting the status
 * at each area, and for querying the current area.
 *
 * The device may operate at one area at a time, as in the case of a mobile device, such as a robot. Other devices may
 * operate at (service) multiple areas simultaneously, as in the case of a sensor that can monitor multiple areas. This
 * cluster specification uses the term "operate" to describe both the operating and servicing actions, regardless of the
 * device type.
 *
 * The cluster allows the client to select one or more areas on the server, to indicate where the device SHOULD attempt
 * to operate. An area is one of a list of options that may be presented by a client for a user choice, or understood by
 * the client, via the semantic data of the area.
 *
...
export declare namespace ServiceArea {
     ...
     */
    export const id: ClusterId & 0x0150;
    ...
    /**
     * Canonical metadata for the ServiceArea cluster.
     *
    ...
    export interface BaseAttributes {
         ...
         *
         * If the SupportedMaps attribute is empty, each entry in this list shall have a unique value for the AreaInfo
         * field and shall have the MapID field set to null.
         *
         ...
         * as interactions with a user interface, or due to internal device changes.
         *
         ...
         * attributes may result in the server setting some or all of them to empty (for SelectedAreas and Progress) or
         * null (for CurrentArea), or updating them with data that matches the constraints from the description of the
         * respective attributes. These actions are required to ensure having a consistent representation of the maps
         * and locations available to the clients.
         *
         ...
         * indicated by the SelectedAreas attribute. For example, a robotic vacuum cleaner may drive without cleaning
         * when traveling without operating.
         *
         ...
         * a camera device may use this attribute to indicate which area it currently takes video of (serviced area) or
         * which area it currently has in view but not taking video of (e.g. an area which is traversed while panning).
         *
         * > [!NOTE]
         *
         ...
         * completion. This may happen, for example, because no operation is in progress or because the completion time
         * is unknown.
         *
    ...
    export interface MapsAttributes {
         ...
         *
         *   - a single level home may be represented using one map for the indoor areas (living room, bedrooms etc.)
         *     and one for the outdoor areas (garden, swimming pool etc.)
         *
         * Each map includes one or more areas - see the SupportedAreas attribute. In the context of this cluster
         * specification, a map is effectively a group label for a set of areas, rather than a graphical representation
         * that the clients can display to the users. The clients that present the list of available areas for user
         * selection (see the SelectAreas command) may choose to filter the SupportedAreas list based on the associated
         * map. For example, the clients may allow the user to indicate that the device is to operate on the first
         * floor, and allow the user to choose only from the areas situated on that level.
         *
         ...
         *
         * When updating the SupportedMaps attribute list by deleting entries, or by setting the attribute to an empty
         * list, the SupportedAreas attribute shall be updated such that all entries in that list meet the constraints
         * indicated in the description of the SupportedAreas attribute. This may result in the server removing entries
     ...
     */
    export interface ProgressReportingAttributes {
        /**
         ...
         *
         *   - the Progress attribute list shall be updated so each entry of SelectedAreas has a matching Progress list
         *     entry, based on the AreaID field
         *
         ...
         *
         * When the device ends operation unexpectedly, such as due to an error, the server shall update all Progress
         * list entries with the Status field set to Operating or Pending to Skipped.
         *
         * When the device finishes operating, successfully or not, it shall NOT change the Progress attribute, except
         * in the case of an unexpected end of operation as described above, or due to changes to the SupportedMaps or
    ...
    /**
     * Commands that may appear in {@link ServiceArea}.
     */
    export interface Commands extends BaseCommands {}

    export type Components = [
        { flags: {}, attributes: BaseAttributes, commands: BaseCommands },
        { flags: { maps: true }, attributes: MapsAttributes },
        { flags: { progressReporting: true }, attributes: ProgressReportingAttributes }
    ];
    ...
    export const Typing: ServiceArea;
}
...
export interface ServiceArea extends ClusterTyping {
    Attributes: ServiceArea.Attributes;
    Commands: ServiceArea.Commands;
    Features: ServiceArea.Features;
    Components: ServiceArea.Components;
}
