 ...
 *
 * This cluster would be supported on IP devices that have a low power mode AND support the ability to be woken up using
 * the Wake on LAN or Wake on WLAN protocol. This cluster provides the device MAC address which is a required input to
 * the Wake on LAN protocol. Besides the MAC address, this cluster provides an optional link-local IPv6 address which is
 * useful to support "Wake on Direct Packet" used by some Ethernet and Wi-Fi devices.
 *
 ...
 * as unicast. If a device provides the link-local address in this cluster, its Ethernet controller or Wi-Fi radio shall
 * respond to the IPv6 neighbor solicitation message for the link-local address without the need to wake host CPU up. In
 * order to receive the magic or neighbor solicitation packets in multicast, the Wi-Fi devices must support Group
...
export declare namespace WakeOnLan {
     ...
     */
    export const id: ClusterId & 0x0503;
    ...
    /**
     * Canonical metadata for the WakeOnLan cluster.
     *
...
export interface WakeOnLan extends ClusterTyping {
    Attributes: WakeOnLan.Attributes;
    Components: WakeOnLan.Components;
}
