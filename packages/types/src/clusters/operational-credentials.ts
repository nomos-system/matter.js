...
import { AccessLevel, OperationalCredentials as OperationalCredentialsModel } from "@matter/model";
import { TlvVendorId, VendorId } from "../datatype/VendorId.js";
import { TlvFabricId, FabricId } from "../datatype/FabricId.js";
...
export namespace OperationalCredentials {
     ...
     *        successfully delivered or until the next transport-layer error, so that the response can be received by
     *        the Administrator invoking the command.
     *
     ...
     *   the existing Fabric associations within the Fabrics list, so that likelihood is as small as possible of a user
     ...
     *   the other Administrative Domain’s party of the removal. Otherwise, users may only observe the removal of a
     ...
     *     attribute entry associated with the accessing fabric index shall be erased and the field shall disappear from
     *     the Fabrics entry.
     *
     *   - If the length of the field’s value is exactly 85, then the VIDVerificationStatement field in the Fabrics
     *     attribute entry associated with the accessing fabric index shall have its value replaced with the value from
     *     the command field.
     *
     * If the VVSC field is present:
     *
     ...
     *     the accessing fabric index shall have its value replaced with the value from the command field. The contents
     *     of the certificate need not be validated by the server. Clients shall validate the contents at time of use.
     *
